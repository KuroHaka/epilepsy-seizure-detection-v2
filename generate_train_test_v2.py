...
train = [
    ...
    "chb10", "chb11", "chb13", "chb14", "chb15", "chb16", "chb17",
...
os.makedirs("data_v2/train", exist_ok=True)
os.makedirs("data_v2/test", exist_ok=True)

DONE = 1
TOTAL = len(seizure_pointers[seizure_pointers["case"].isin(train + test)]["seizure_file"].unique())
...
for target in train:
    ...
    process_patient(patient, f"data_v2/train/{target}.npz")

print("\nProcessing test patients...")
for target in test:
    print(f"Patient: {target}")
    patient = seizure_pointers[seizure_pointers["case"] == target]
    process_patient(patient, f"data_v2/test/{target}.npz")
